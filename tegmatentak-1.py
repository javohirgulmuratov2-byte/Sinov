...
def db_use_promo_code(code):
    """
    Promokodni bir martalik qilib ishlatadi: topilsa summasini qaytaradi va o'chiradi.
    Bir vaqtda bir nechta odam yuborsa ham faqat bittasi muvaffaqiyatli bo'lishi uchun
    DELETE...RETURNING mantig'iga yaqin: avval o'qiymiz, keyin darhol o'chiramiz.
    """
...
async def call_groq_text(messages: list, max_tokens: int = 2048) -> str:
    """Groq matn chaqiruvi. reasoning_format='hidden' bilan urinadi, agar kutubxona
    versiyasi bu parametrni tanimasa (eski groq SDK), parametrsiz qayta uradi.
    Har doim <think>...</think> blokini qo'shimcha tozalab tashlaydi (ehtiyot chorasi)."""
    try:
        response = await asyncio.to_thread(
            groq_client.chat.completions.create,
        ...
        logging.warning(f"reasoning_format qollab-quvvatlanmadi, parametrsiz qayta urinilmoqda: {e}")
        response = await asyncio.to_thread(
            groq_client.chat.completions.create,
            model=GROQ_TEXT_MODEL,
            messages=messages,
...
async def promo_code_handler(message: Message, state: FSMContext):
    ...
    amount = db_use_promo_code(message.text.strip())
    if amount is None:
        return  # Notogri promokod - hech narsa demaymiz
...
async def ask_ai_assistant(question: str, is_admin: bool = False) -> tuple:
    ...
    try:
        ...
        clean_text = remove_apostrophe_letters(clean_text).strip()
        if not clean_text:
